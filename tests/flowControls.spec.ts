...
describe('Flow Controls', () => {
  ...
  describe('ifCondition', () => {
    it('should execute the sequence if condition is true', async () => {
      ...
      const result = await fc.ifCondition({ condition: '${value} > 3' }, mockRunSequence, mockRunAlternativeSequence);
  ...
  describe('whileCondition', () => {
    xit('should execute the sequence while condition is true', async () => {
      const mockRunSequence = jest.fn().mockResolvedValue({ success: true });
      (store.getValue as jest.Mock).mockReturnValueOnce('3').mockReturnValueOnce('2').mockReturnValueOnce('1');

      const result = await fc.whileCondition({ condition: '${value} > 0' }, mockRunSequence, jest.fn());
  ...
  describe('repeatXTimes', () => {
    it('should repeat the sequence x times', async () => {
      ...
      const result = await fc.repeatXTimes({ x: '3' }, mockRunSequence, jest.fn());
  ...
  describe('repeatXTimesWithIndex', () => {
    it('should repeat the sequence x times and set the index variable', async () => {
      ...
      const result = await fc.repeatXTimesWithIndex({ x: '3', index: 'i' }, mockRunSequence, jest.fn());
  ...
  describe('tryCatch', () => {
    it('should execute the sequence and return success if no error occurs', async () => {
      ...
      const result = await fc.tryCatch({}, mockRunSequence, mockRunAlternativeSequence);
  ...
  describe('forEachLineOfFile', () => {
    it('should execute the sequence for each non-empty line in the file', async () => {
      ...
      const result = await fc.forEachLineOfFile({ filename: 'test.txt' }, mockRunSequence, jest.fn());
    ...
    it('should return an error if the file does not exist', async () => {
      ...
      const result = await fc.forEachLineOfFile({ filename: 'nonexistent.txt' }, jest.fn(), jest.fn());
  ...
  describe('forEachItemInList', () => {
    it('should execute the sequence for each item in the list', async () => {
      ...
      const result = await fc.forEachItemInList({ listVariable: 'myList', itemVariable: 'currentItem' }, mockRunSequence, jest.fn());
    ...
    it('should return an error if the list variable is not found', async () => {
      ...
      const result = await fc.forEachItemInList({ listVariable: 'nonexistentList', itemVariable: 'currentItem' }, jest.fn(), jest.fn());

      expect(result.success).toBe(false);
      expect(result.message).toContain('Variable nonexistentList not found');
    });
  });
  ...
  describe('Error handling', () => {
    it('should return an error for invalid repeat count', async () => {
      const result = await fc.repeatXTimes({ x: 'invalid' }, jest.fn(), jest.fn());

      expect(result.success).toBe(false);
      expect(result.message).toContain('Invalid number of times to repeat');
    });

    it('should return an error for missing condition in if statement', async () => {
      const result = await fc.ifCondition({}, jest.fn(), jest.fn());

      expect(result.success).toBe(false);
      expect(result.message).toContain('No condition provided');
    });

    it('should return an error for invalid condition in while loop', async () => {
      (store.getValue as jest.Mock).mockReturnValue('invalid');

      const result = await fc.whileCondition({ condition: '${value} > 3' }, jest.fn(), jest.fn());

      expect(result.success).toBe(false);
      expect(result.message).toContain('Failed to evaluate condition');
    });
  });
});